...
public class EDUInterface extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ("start".equals(e.getActionCommand())) {
			...
			messagesTextArea.append("\n\nUPC Product " + upcTextField.getText()
					+ " added.");
			...
			messagesTextArea.append("\n\nBIC Product " + bicTextField.getText()
					+ " added. ");
		...
		// }
		/*
		 * catch (AddWhileBaggingException awbe) { messagesTextArea.setText(
		 * "EXCEPTION: Item scanned before previous item is bagged."); } catch
		 * (AddWhilePayingException awpe) { messagesTextArea.setText(
		 * "EXCEPTION: Item scanned while payment is being processed"); } catch
		 * (InvalidProductException ipe) {
		 * messagesTextArea.setText("EXCEPTION: Item not recognized."); } catch
		 * (IncorrectStateException ise) { messagesTextArea.setText(
		 * "EXCEPTION: Invalid action for current state of Self Check Out."); }
		 ...
		 * iupce.getMessage()); } catch (InvalidBICException ibice) {
		 * messagesTextArea.setText("EXCEPTION: Invalid BIC: " +
		 * ibice.getMessage()); } catch (NullPointerException npe) { //Checks if
		 * the selfCheckOut object is null if (selfCheckOut == null) {
		 * messagesTextArea
		 * .setText("EXCEPTION: Self Check Out has not been started."); } else {
		 ...
		 * exception.getMessage() +". Check console for more details.");
